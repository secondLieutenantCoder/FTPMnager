...
- (void)openRead:(GRRequest *)request
{
    ...
    NSString * createFlag = [defaults objectForKey:@"WWcreateDirectory"];
    if (createFlag) {
        // 相对路径
        NSString * pathStr = [defaults objectForKey:@"WWencodeCreatePath"];
        // host
        NSDictionary * accountDic = [defaults objectForKey:@"WWAccount"];
        NSString * serverStr = accountDic[@"server"];
        NSString * fullUrlStr = [NSString stringWithFormat:@"%@%@",serverStr,pathStr];
        fullUrl = [NSURL URLWithString:fullUrlStr];
    }else{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, timeout * NSEC_PER_SEC), dispatch_get_local_queue(), ^{
        if (!request.didOpenStream && request.error == nil)
        {
            dispatch_sync(dispatch_get_main_queue(), ^{
                [SVProgressHUD showInfoWithStatus:@"服务器连接超时"];
